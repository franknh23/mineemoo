...
var initOSCModules = function()
{
    ...
    else if (typeof OSCheckout != 'undefined')
    {
        OSCheckout.prototype.__placeOrder = OSCheckout.prototype.placeOrder;
        // AheadWorks OneStepCheckout 1.3.5
        
        OSCheckout.prototype.placeOrder = function()
        {
            if($('p_method_cryozonic_stripe')){
                if (!$('p_method_cryozonic_stripe').checked)
                    return this.__placeOrder();
            }else{
                return this.__placeOrder();
            }
            var me = this;
            createStripeTokenAC(function(err)
            {
                if (err && err == three_d_secure_canceled)
                    console.error(three_d_secure_canceled);
                else if (err)
        ...
        OSCheckout.prototype._savePaymentMethods = OSCheckout.prototype.savePaymentMethods;
        OSCheckout.prototype.savePaymentMethods = function() {
            if ($('p_method_cryozonic_stripe') && $('p_method_cryozonic_stripe').checked)
                return;
            else
                return this._savePaymentMethods();
        };
        cryozonicStripe.oscInitialized = true;
    }
...
var createStripeTokenAC = function(callback)
{
    var done = function(err)
    {
        return callback(err);
