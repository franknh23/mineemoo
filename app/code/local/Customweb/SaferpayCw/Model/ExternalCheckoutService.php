...
class Customweb_SaferpayCw_Model_ExternalCheckoutService implements Customweb_Payment_ExternalCheckout_ICheckoutService {
	...
	public function authenticate(Customweb_Payment_ExternalCheckout_IContext $context, $emailAddress, $successUrl){
		...
		if ($quote->isAllowedGuestCheckout() && Mage::getStoreConfig('saferpaycw/general/external_checkout_account_creation') == 'skip_selection' && !empty($emailAddress)) {
			$customerData = array(
				'email' => $emailAddress,
			...
			if ($dateOfBirth != null) {
				$customerData['dob'] = Mage::helper('SaferpayCw/externalCheckout')->getDateAsString($dateOfBirth);
			}
			...
			$result = Mage::helper('SaferpayCw/externalCheckout')->validateCustomerData($quote, $customerData, 'guest');
			if ($result !== true) {
	...
	public function renderAdditionalFormElements(Customweb_Payment_ExternalCheckout_IContext $context, $errorMessage){
		...
		$block = Mage::getSingleton('core/layout')->createBlock('saferpaycw/externalCheckout_additionalInformation')->setContext($context);
		return $block->toHtml();
	}
	...
	public function processAdditionalFormElements(Customweb_Payment_ExternalCheckout_IContext $context, Customweb_Core_Http_IRequest $request){
		...
		if (Mage::helper('SaferpayCw/externalCheckout')->isGenderRequired($context->getQuote())) {
		...
		if (Mage::helper('SaferpayCw/externalCheckout')->isDateOfBirthRequired($context->getQuote())) {
...
// 			}
		...
		if (!empty($paymentMethod)) {
			...
			$quote->setSaferpayCwExternalCheckout(true);
