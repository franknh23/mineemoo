...
class Customweb_SaferpayCw_Model_Observer
{
				...
				foreach ($productIds as $productId) {
					...
					if (!$product->isSalable()) {
						foreach ($orders as $order) {
							...
							$order->setIsActive(0);
							$order->addStatusToHistory(Customweb_SaferpayCw_Model_Method::SAFERPAYCW_STATUS_CANCELED, Mage::helper('SaferpayCw')->__('Order cancelled, because the ordered products are not available.'));
							$order->save();
						}
						break;
					}
	...
	{
		if (!Mage::registry('customweb_saferpaycw_transaction_resolver_registered')) {
			Customweb_I18n_Translation::getInstance()->addResolver(new Customweb_SaferpayCw_Model_TranslationResolver());
			Mage::register('customweb_saferpaycw_transaction_resolver_registered', true);
		}
	}

	public function registerLoggingListener(Varien_Event_Observer $observer)
	{
		if (!Mage::registry('customweb_saferpaycw_logging_listener_registered')) {
			Customweb_Core_Logger_Factory::addListener(new Customweb_SaferpayCw_Model_LoggingListener());
			Mage::register('customweb_saferpaycw_logging_listener_registered', true);
		}
	}
	...
	public function migrateCustomerTransactions(Varien_Event_Observer $observer)
	{
		...
		Mage::helper('SaferpayCw')->migrateCustomersTransactions($customer);
	}
}
