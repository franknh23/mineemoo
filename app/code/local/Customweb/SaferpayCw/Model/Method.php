...
abstract class Customweb_SaferpayCw_Model_Method extends Mage_Payment_Model_Method_Abstract implements
	...
	public function isAvailable($quote = null){
		...
		if ($isAvailable && $quote !== null && $quote->getSaferpayCwExternalCheckout() !== true) {
	...
	public function validate(){
		...
		$arguments = null;
		return Customweb_Licensing_SaferpayCw_License::run('kku1qsu6q37v4ka0', $this, $arguments);
	}

	final public function call_a47vt6ig73d12bqu() {
	...
	public function cancel(Varien_Object $payment){
		...
		try {
			if (Mage::registry('saferpaycw_update_transaction') != null) {
				$transaction = Mage::registry('saferpaycw_update_transaction');
			}
			else {
	...
	public function refund(Varien_Object $payment, $amount){
		...
		try {
			...
			if ($refunded || Mage::registry('cw_saferpaycw_refund_update') === true) {
				...
				$payment->getCreditmemo()->setSaferpaycwRefundId($refund->getRefundId());
		...
		if ($order->getId()) {
			...
			if ($transaction->getTransactionObject()->isAuthorized()) {
				...
				if ($transaction->getTransactionObject()->isAuthorizationUncertain()) {
					...
					$order->addStatusHistoryComment(
							$this->getHelper()->__('Customer successfully returned from Saferpay') . "\n<br>PaymentId:" .
									 $transaction->getTransactionObject()->getPaymentId());
				}
				$order->save();
	 ...
	 */
	public function createInvoiceAfterUncertain(Customweb_SaferpayCw_Model_Transaction $transaction){
	 ...
	 */
	public function createInvoice(Mage_Sales_Model_Order $order, Customweb_SaferpayCw_Model_Transaction $transaction){
		$result = false;
	...
	public function getOrderPlaceRedirectUrl(){
		...
		if ($adapter instanceof Customweb_Payment_Authorization_PaymentPage_IAdapter) {
			return Mage::getUrl('SaferpayCw/process/ppRedirect',
					array(
		...
		elseif ($adapter instanceof Customweb_Payment_Authorization_Iframe_IAdapter ||
			...
			return Mage::getUrl('SaferpayCw/checkout/pay',
					array(
						'transaction_id' => $transactionId,
						'secret' => $secretHash,
						'_secure' => true
	...
	public function fail(Customweb_SaferpayCw_Model_Transaction $transaction, array $parameters){
		...
		if ($order->getState() == Mage_Sales_Model_Order::STATE_PENDING_PAYMENT &&
			...
			//$order->setIsActive(0);
			$this->setOrderStatus($order, Customweb_SaferpayCw_Model_Method::SAFERPAYCW_STATUS_CANCELED,
	...
	public function generateMotoForm(Mage_Sales_Model_Order $order, Customweb_SaferpayCw_Model_Transaction $transaction, $parameters){
		...
		$cancelUrl = Mage::getUrl('SaferpayCw/process/motoCancel', array(
			'_secure' => true,
		...
		elseif (isset($fileKeys[$key])) {
			...
			if (empty($rs) || $defaultValue == $rs) {
				...
				return new Customweb_Core_Stream_Input_File(
						Mage::getBaseDir('media') . '/saferpaycw/setting/' . $this->getPaymentMethodName() . '/' . $key . '/' . $rs);
			}
		}
	...
	public function existsPaymentMethodConfigurationValue($key, $languageCode = null){
		...
		return null != Mage::getStoreConfig('payment/' . $this->getCode() . '/' . $key,
				Customweb_SaferpayCw_Model_ConfigurationAdapter::getStoreId());
	}
	...
	public function updateTransaction(Customweb_SaferpayCw_Model_Transaction $transaction, Mage_Sales_Model_Order $order, $moto = false){
		if (!$transaction->getOrderId() && $order->getId()) {
			...
			$session->unsSaferpayCwCheckoutId();
	 ...
	 */
	public function getTransactionContext(Mage_Sales_Model_Order $order, Customweb_SaferpayCw_Model_Transaction $transaction, $backendSuccessUrl = '', $backendFailUrl = '', $backendCancelUrl = ''){
		$aliasTransactionId = Mage::getSingleton('checkout/session')->getAliasId();
		$storeId = $order->getStore()->getId();
		$orderContext = Customweb_SaferpayCw_Model_OrderContext::fromOrder($order);
		$transactionContext = new Customweb_SaferpayCw_Model_TransactionContext($orderContext, $order->getIncrementId(),
	...
	public function getMotoTransactionContext(Mage_Sales_Model_Order $order, Customweb_SaferpayCw_Model_Transaction $transaction){
		...
		$backendSuccessUrl = Mage::getModel('adminhtml/url')->getUrl('adminhtml/motosaferpaycw/success',
				array(
					'order_id' => $order->getId()
				));
		$backendFailUrl = Mage::getModel('adminhtml/url')->getUrl('adminhtml/motosaferpaycw/fail',
				array(
					'order_id' => $order->getId()
				));
		$backendCancelUrl = Mage::getModel('adminhtml/url')->getUrl('adminhtml/motosaferpaycw/cancel',
				array(
					'order_id' => $order->getId()
				));
	...
	public function loadAliasForCustomer(){
		...
		if (Mage::helper('customer')->isLoggedIn()) {
			...
			$collection = Mage::getModel('saferpaycw/transaction')->getCollection()->addFieldToFilter('customer_id',
					array(
						'eq' => $customerId
