...
class Customweb_SaferpayCw_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	{
		if ($customer->getData('saferpaycw_migrated')) {
			return;
		}
		...
		$payments = Mage::getModel('sales/order_payment')->getCollection()
			...
			->addFieldToFilter('method', array('like' => 'saferpaycw_%'));
		...
		$query = 'UPDATE ' . Mage::getSingleton('core/resource')->getTableName('customer_entity')
			. ' SET saferpaycw_migrated = 1, created_at = ' . $connection->quote(Varien_Date::formatDate($customer->getCreatedAt()))
