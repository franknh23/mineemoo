...
class Customweb_SaferpayCw_Block_Form extends Mage_Payment_Block_Form {
	...
	public function getContent(){
		...
		$arguments = null;
		return Customweb_Licensing_SaferpayCw_License::run('ia6r6ek104srjeai', $this, $arguments);
	}

	final public function call_1n0onc2c53j59m56() {
	...
	public function getProcessUrl(){
		return Mage::getUrl('SaferpayCw/process/process', array(
			'_secure' => true
		));
	}
	...
	public function getAliasSelect(){
		...
		if ($payment->getPaymentMethodConfigurationValue('alias_manager') == 'active') {
			...
			if (count($aliasList)) {
				...
				// on select fields.       	    	 			    	 
				$selectControl = new Customweb_SaferpayCw_Model_Select("alias_select", $alias, 'new',
						"cwpm_" . $payment->getCode() . ".loadAliasData(this)");
				$aliasElement = new Customweb_Form_Element(Mage::helper('SaferpayCw')->__("Saved cards:"), $selectControl,
						Mage::helper('SaferpayCw')->__("You may choose one of the cards you paid before on this site."));
