...
class Customweb_SaferpayCw_CheckoutController extends Customweb_SaferpayCw_Controller_Action
{
	public function breakoutAction()
	{
		...
		if ($transaction->getTransactionObject()->isAuthorizationFailed()) {
			...
			$redirectionUrl = Mage::getUrl('SaferpayCw/process/success', array('transaction_id' => $transaction->getTransactionId(), 'secret' => $secretHash, '_secure' => true));
		}
	...
	public function payAction()
	{
		try{
			...
			if($transaction->getId()){
				...
				if(!$transaction->getTransactionObject()->isAuthorizationFailed()){
					if(!$transaction->getTransactionObject()->isAuthorized()) {
						...
						if($transaction->getTransactionObject()->isAuthorizationFailed()){
							$secretHash = $this->getHelper()->hash($transaction->getTransactionId());
							$this->_redirect('SaferpayCw/process/fail',array('transaction_id' => $transaction->getTransactionId(), 'secret' => $secretHash, '_secure' => true));
						}
						else {
							$this->renderLayout();
						}
			...
			$this->getLayout()->getBlock('root')->setTemplate('page/1column.phtml');
			$this->getLayout()->getBlock('content')->append(
					$this->getLayout()->createBlock('saferpaycw/expired')
					);
			$this->renderLayout();
		}
	}
