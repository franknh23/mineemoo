...
class TM_FireCheckout_Helper_Deliverydate extends Mage_Core_Helper_Abstract
{
    ...
    public function getTimeRangeStrings()
    {
        if (null === $this->_timeRangeStrings) {
            $ranges = Mage::getStoreConfig('firecheckout/delivery_date/time_range');
            $model = Mage::getModel('tmcore/adminhtml_system_config_backend_serialized_array');
            $model->setValue($ranges)->afterLoad(); // unserialize raw value
            $ranges = $model->getValue();
    ...
    public function getExcludedDateStrings()
    {
        if (null === $this->_excludedDateStrings) {
            $dates = Mage::getStoreConfig('firecheckout/delivery_date/excluded_dates');
            $model = Mage::getModel('tmcore/adminhtml_system_config_backend_serialized_array');
            $model->setValue($dates)->afterLoad(); // unserialize raw value
            $dates = $model->getValue();
